...
# Cargar el modelo de spaCy con caché para evitar recargarlo en cada ejecución
@st.cache_resource
...
def extract_terms_pos(text):
    """Extrae términos basados en etiquetas gramaticales (POS Tagging)."""
    fragmentos = dividir_texto(text)
    term_counts = Counter()

    for fragmento in fragmentos:
        doc = nlp(fragmento.lower())  # Normalizar a minúscula
        ...
        for i, token in enumerate(doc):
            if token.pos_ == "NOUN":
                ...
                token.pos_ == "NOUN"
                and i + 2 < len(doc)
                and doc[i + 1].pos_ == "ADP"
                and doc[i + 1].text.lower() == "of"
                and doc[i + 2].pos_ == "NOUN"
            ):
